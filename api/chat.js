...
// ── Coach Accounts ─────────────────────────────────────────────────────────
// ── Fallback coaches (used if Sheet not connected) ───────────────────────────
const FALLBACK_COACHES = [
  { name:"Alicia Williams (Demo)", password:"RICportal2025", role:"Lead Coach" },
  { name:"EF & Tutoring (Demo)",   password:"RICportal2025", role:"EF & Tutoring Coach" },
  { name:"Ed Therapist (Demo)",    password:"RICportal2025", role:"ED Therapist" },
  { name:"Test Prep (Demo)",       password:"RICportal2025", role:"Test Prep Specialist" },
];

async function fetchCoaches() {
  if (!SHEET_API_URL || SHEET_API_URL === "YOUR_APPS_SCRIPT_URL_HERE") return FALLBACK_COACHES;
  try {
    const data = await fetchSheetAPI({ action:"getCoaches" });
    if (data && data.coaches && data.coaches.length > 0) return data.coaches;
    return FALLBACK_COACHES;
  } catch(e) { return FALLBACK_COACHES; }
}
...
function Login({onLogin}) {
  ...
  const [_loaded]=useState(()=>{
    fetchCoaches().then(c=>{if(c&&c.length>0)setCoaches(c);});
    return true;
  });
